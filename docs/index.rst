...
That's it! ``pyngrok`` is now available `as a package to your Python projects <#create-an-endpoint>`_,
and ``ngrok`` is now available `from the command line <#command-line-usage>`_.

Create an Endpoint
==================

To create an endpoint, use the :func:`~pyngrok.ngrok.connect_endpoint` method, which returns a
:class:`~pyngrok.ngrok.NgrokEndpoint` with a reference to the public URL assigned by ``ngrok`` in its
:attr:`~pyngrok.ngrok.NgrokEndpoint.url` attribute.

.. code-block:: python

    from pyngrok import ngrok

    # Create an endpoint forwarding to a local HTTP service
    # <NgrokEndpoint: "https://<public_sub>.ngrok.dev" -> "http://localhost:8000">
    endpoint = ngrok.connect_endpoint(upstream="http://localhost:8000")

    # Create a named endpoint with pooling enabled
    pooled = ngrok.connect_endpoint(upstream="http://localhost:9000",
                                    url="https://some-endpoint.internal",
                                    pooling_enabled=True)

    # Create an endpoint from a definition in ngrok's config file (an entry under
    # the `endpoints:` block; note that "-api" will be appended to its name when started)
    named = ngrok.connect_endpoint(name="my-config-file-endpoint")

The :func:`~pyngrok.ngrok.connect_endpoint` method takes ``kwargs`` as well, which are forwarded to the
`ngrok endpoint API <https://ngrok.com/docs/agent/api/#create-endpoint>`_ (e.g. ``traffic_policy``,
``description``, ``metadata``).

Get Active Endpoints
--------------------

Use :func:`~pyngrok.ngrok.get_endpoints` to ask the ``ngrok`` agent which endpoints are currently running.
...
.. code-block:: python
    ...
    # [<NgrokEndpoint: "https://<public_sub>.ngrok.dev" -> "http://localhost:8000">]
    endpoints = ngrok.get_endpoints()

Stop an Endpoint
----------------

All active endpoints will automatically be closed when the Python process terminates, but you can also stop
them manually with :func:`~pyngrok.ngrok.disconnect_endpoint`. Endpoints are identified by ``name``.

.. code-block:: python

    from pyngrok import ngrok

    ngrok.disconnect_endpoint(endpoint.name)
...
.. note::
    ...
    ``ngrok`` has deprecated its tunnels API in favor of `endpoints <#create-an-endpoint>`_. ``pyngrok`` continues
